...
(function () {
    ...
    function addPhotoItem(e) {
        ...
        if (e.data == 0)
        {
            ...
            ShowAndTell.ListView.data.push({ title: photoItem[0].title, text: photoItem[0].date, picture: photoItem[0].thumbURL, bigPicture: photoItem[0].itemURL });
        }
         ...
         //   ShowAndTell.Map.addPushpin(item[0].latProp, item[0].lonProp, item[0].title, item[0].date);
            ...
            ShowAndTell.ListView.data.push({ title: item[0].title, text: item[0].date, picture: item[0].thumbURL, bigPicture: item[0].itemURL });
    ...
    var page = WinJS.UI.Pages.define("/pages/PhotosList/PhotosList.html", {
        ready: function (element, options) {
           
        window.appInsights = appInsights;
        appInsights.trackPageView("PhotosList Page");
        ...
        {
            getFilesWorker = new Worker('/pages/PhotosList/getFilesWorker.js');
    ...
    function invokePhotoItem(eventObject) {
        ...
        eventObject.detail.itemPromise.done(function (invokedItem) {
            if (invokedItem.data.bigPicture != null) {
                ShowAndTell.DoodlePage.imageFile = invokedItem.data.bigPicture;
                WinJS.Navigation.navigate("/pages/DoodlePage/DoodlePage.html", invokedItem.data.bigPicture);
           //     postMessage({ 7845: invokedItem.data.bigPicture },"*");
            }
            else {
                var msg = new Windows.UI.Popups.MessageDialog("The selected item cannot be shown.");
